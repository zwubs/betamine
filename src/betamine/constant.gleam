import betamine/common/entity/player/player_game_mode
import betamine/common/math/vector3.{Vector3}
import betamine/common/text_component

// Server Constants
pub const default_server_interface = "0.0.0.0"

pub const default_server_port = 25_565

// Mojang Constants
pub const mojang_base_profile_host = "sessionserver.mojang.com"

pub const mojang_base_profile_path = "session/minecraft/profile"

// Minecraft Constants
pub const mc_version_name = "1.21.11"

pub const mc_version_protocol = 774

pub const mc_max_player_count = 10

// Same as MOTD (message of the day)
pub const mc_server_description = text_component.TextComponent(
  "Hello Betamine!",
)

pub const mc_favicon = "data:image/png;base64,"

pub const mc_view_distance = 8

pub const mc_player_game_mode = player_game_mode.Creative

pub const mc_simulation_distance = 2

pub const mc_player_spawn_point = Vector3(0.0, 96.0, 0.0)

pub const mc_world_chunk_sections_per_chunk = 24

pub const mc_world_chunk_offset = -4
